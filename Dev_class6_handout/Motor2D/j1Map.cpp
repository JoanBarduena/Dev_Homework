...
void j1Map::Draw()
{
	...
	for (uint y = 0; y < layer->height; y++)
	{
		...
		{
			uint id = layer->Get(x, y);
			uint gid = layer->data[id];

			if (gid != 0)
			{
				SDL_Rect rect = tileset->GetTileRect(gid);
				//iPoint pos = MapToWorld(x, y); // With the function POS. 
...
{
	iPoint ret(0,0);
	// TODO 2: Add orthographic world to map coor dinates
	/*ret.x = x / data.tile_width;
	ret.y = y / data.tile_height;*/

	// TODO 3: Add the case for isometric maps to WorldToMap
	ret.x = (2*x / data.tile_width + 2*y / data.tile_height) / 2;
	ret.y = (2*y / data.tile_height - (2*x / data.tile_width)) / 2;
