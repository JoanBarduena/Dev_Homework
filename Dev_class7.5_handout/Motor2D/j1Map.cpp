...
void j1Map::PropagateBFS()
{
	...
	if (frontier.Count() > 0)
	{
		...
		if (visited.find(pos1) == -1) 
		{
			if (IsWalkable(pos1.x, pos1.y) == true)
			{
				frontier.Push(pos1);
				visited.add(pos1);
			}
		...
		if (visited.find(pos2) == -1)
		{
			if (IsWalkable(pos2.x, pos2.y) == true)
			{
				frontier.Push(pos2);
				visited.add(pos2);
			}
		...
		if (visited.find(pos3) == -1)
		{
			if (IsWalkable(pos3.x, pos3.y) == true)
			{
				frontier.Push(pos3);
				visited.add(pos3);
			}
		...
		if (visited.find(pos4) == -1)
		{
			if (IsWalkable(pos4.x, pos4.y) == true)
			{
				frontier.Push(pos4);
				visited.add(pos4);
			}
...
bool j1Map::IsWalkable(int x, int y) const
{
	...
	if (y < 0 || x > data.width - 1 || x < 0 || y > data.height - 1 || data.layers.start->next->data->Get(x,y) != 0) return false; 
